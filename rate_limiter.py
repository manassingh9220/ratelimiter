...
from script import LUA_FIXED_WINDOW, LUA_SLIDING_COUNTER, LUA_SLIDING_LOG
...
if __name__ == "__main__":
    now = time.time()          # one timestamp for all three, so they compare fairly
    r.delete("rl:log:demo", "rl:swc:demo", f"rl:demo:{int(now) // 60}")

    print(f"{'#':>3}  {'fixed':<16}{'log':<16}{'counter':<16}")
    print("-" * 55)
    for i in range(12):
        ...
        print(f"{i + 1:>3}  {fmt(f):<16}{fmt(l):<16}{fmt(c):<16}")